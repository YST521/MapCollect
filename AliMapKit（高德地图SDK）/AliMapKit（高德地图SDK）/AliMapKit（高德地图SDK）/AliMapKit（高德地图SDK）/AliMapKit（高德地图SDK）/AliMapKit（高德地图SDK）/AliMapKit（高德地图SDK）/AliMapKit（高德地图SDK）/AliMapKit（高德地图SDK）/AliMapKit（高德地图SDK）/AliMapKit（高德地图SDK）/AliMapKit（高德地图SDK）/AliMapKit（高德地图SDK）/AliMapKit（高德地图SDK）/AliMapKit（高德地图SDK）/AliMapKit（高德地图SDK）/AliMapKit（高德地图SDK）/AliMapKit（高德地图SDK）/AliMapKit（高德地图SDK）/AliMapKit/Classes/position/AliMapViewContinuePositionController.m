...
-(void)setupAllSubViews{
    ...
    self.adressLa =[[UILabel alloc]init];
    self.adressLa.frame = CGRectMake(0, SCREEN_HEGHT-60, SCREEN_WIDTH, 40);
    self.adressLa.textAlignment = NSTextAlignmentCenter;
    [self.view  addSubview:self.adressLa];
...
-(void)showLocationInfomation:(CLLocation *)location{
    ...
    [self getAdressCode:[NSString stringWithFormat:@"%f",location.coordinate.longitude] latitue:[NSString stringWithFormat:@"%f",location.coordinate.latitude]];
}

//经纬度-> 地名
-(void)getAdressCode:(NSString*)lon latitue:(NSString*)la{
    
    //1.获得输入的经纬度
    NSString *longtitudeText=lon;
    NSString *latitudeText=la;
    ...
    [self.geocoder reverseGeocodeLocation:location completionHandler:^(NSArray *placemarks, NSError *error) {
        ...
        }else//编码成功
        {
            ...
            self.adressLa.text =firstPlacemark.name;
